...
Rules: plan complex missions for an initial capacity of up to 12 concurrent child agents, then scale only within the capacity supported by the project OMX configuration, independent work lanes, model budget, integration bandwidth, and machine resources. Twelve is not a permanent hard cap, and it is not a target to fill when fewer bounded lanes are useful. Child prompts remain under AGENTS.md authority; prefer inherited model defaults unless a task has a concrete model reason; `worker` is a team-runtime surface, not a general-purpose child role.
Project OMX safety: treat `[agents].max_threads` as a global fleet budget, never as a per-parent allowance. Copy the tracked `.codex/config.template.toml` baseline into the ignored local `.codex/config.toml` before project setup. Before running `omx setup --scope project --merge-agents`, stop active teams and use a clean disposable worktree; inspect the resulting `AGENTS.md` and local config diff before adopting it. Treat dry-run output as advisory, and never commit session-specific worker overlays, local paths, runtime state, logs, or credentials.
</child_agent_protocol>
